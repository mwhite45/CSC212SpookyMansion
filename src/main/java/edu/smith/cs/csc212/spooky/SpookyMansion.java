...
public class SpookyMansion implements GameWorld {
	...
	public SpookyMansion() {
		...
		entranceHall.addExit(new SecretExit("crypt", "secret passage"));
		entranceHall.addItem("key");
		entranceHall.addItem("carpet");
		entranceHall.addExit(new LockedExit("crypt", "There is a locked door."));
		...
		basement.addExit(new Exit("room2", "There is a room here."));
		//secret exit to secret room
		basement.addExit(new SecretExit("secretroom", "You found the secret room."));
		...
		attic.addExit(new Exit("room1", "There is a room here."));
		...
		//Add an exit from secretRoom to the basement 
		secretRoom.addExit(new Exit("basement", "There is an exit to the basement."));
		
		//add room 1
		Place room1 = insert(
				Place.create("room1", "You have found a room connected to the attic.\n" + "It is ....\n"));
		//add room 2
		Place room2 = insert(
				Place.create("room2", "You have found a room connected to the basement.\n" + "It is ....\n"));

		int hallwayDepth = 4; //make hallway even longer, increased depth by 1
		...
		for (int i = 0; i < hallwayDepth; i++) {
			Place hallwayPart = insert(Place.create("hallway" + i, "This is a very long hallway."
					+ "There's a " + i + " scratched on the wall."));
			if (i == 0) {
